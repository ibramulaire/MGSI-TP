...
std::vector<Sommet> &pointh = ch.getCurveVertices();
std::vector<Sommet> &rtangenteh = fh.getvecTangent();
std::vector<Sommet> &rNormaleh = fh.getvecNormale();
std::vector<Sommet> &rOxulateurh = fh.getVecOxulateur();
std::vector<Cercle> &Cerclesh = fh.getCercles();
std::vector<Cercle> &Cerclesextrudh = fh.getCerclesBalayant();
void calcules()
{
  c.TousCalculer();
  ch.TousCalculer();
...
void displayCourbeh(void)
{
  std::vector<Sommet> pointcercle = Cerclesh[t].Point;
  for (int i = 0; i < n - 1; i++)
  {
    // la courbe
    glBegin(GL_LINES);
    glColor3f(1.0, 0.0, 0.0);
    glVertex3d(pointh[i].x, pointh[i].y, pointh[i].z);
    glVertex3d(pointh[i + 1].x, pointh[i + 1].y, pointh[i + 1].z);
    glEnd();
    // le cercle oxu
    glBegin(GL_LINES);
    glColor3f(0.0, 0.0, 1.0);
    ...
    glVertex3d(pointcercle[i + 1].x, pointcercle[i + 1].y, pointcercle[i + 1].z);
    glEnd();
    std::vector<Sommet> pointcercleextrud = Cerclesextrudh[i].Point;
    for (int j = 0; j < n - 1; j++)
    {
  ...
  glBegin(GL_LINES);
  glColor3f(1.0, 0.0, 0.0);
  glVertex3d(pointh[t].x, pointh[t].y, pointh[t].z);
  glVertex3d(pointh[t].x + rtangenteh[t].x, pointh[t].y + rtangenteh[t].y, pointh[t].z + rtangenteh[t].z);
  glEnd();

  glBegin(GL_LINES);
  glColor3f(0.0, 1.0, 0.0);
  glVertex3d(pointh[t].x, pointh[t].y, pointh[t].z);
  glVertex3d(pointh[t].x + rNormaleh[t].x, pointh[t].y + rNormaleh[t].y, pointh[t].z + rNormaleh[t].z);
  glEnd();

  glBegin(GL_LINES);
  glColor3f(0.0, 0.0, 1.0);
  glVertex3d(pointh[t].x, pointh[t].y, pointh[t].z);
  glVertex3d(pointh[t].x + rOxulateurh[t].x, pointh[t].y + rOxulateurh[t].y, pointh[t].z + rOxulateurh[t].z);
  glEnd();
}
