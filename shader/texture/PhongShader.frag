...
void main() 
{
  ...
  //  difuse =vec4(light.intensities,1.0)*color;
   ...
   else
   {
    ...
    finalColor=bordcolor*mix(texture(ourTexture0,TexCoord),texture(ourTexture1,TexCoord),silhouette);
    else
    finalColor=(difuse+ambient+specular)*mix(texture(ourTexture1,TexCoord),texture(ourTexture0,TexCoord),silhouette);
