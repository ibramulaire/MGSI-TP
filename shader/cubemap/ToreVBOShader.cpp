...
GLuint MatrixIDView_SKY,MatrixIDPerspective_SKY;    // handle pour la matrice MVP
GLuint VBO_sommets,VBO_normales, VBO_indices,VBO_UVtext,VBO_SKY,VAO_SKY,VAO;
...
GLuint locationTexture0,locationTexture1,locationNormalMap,locationTextureSKY;
//-------------------------
...
//-----------------------------------------
{
  ...
  */ 
  ...
  locationTextureSKY = glGetUniformLocation(programID2, "skybox");
}
//----------------------------------------
...
// le shader
   ...
   programID2 = LoadShaders( "skyshader.vert", "skyshader.frag" );
  ...
  MatrixIDView_SKY = glGetUniformLocation(programID2, "view");
  MatrixIDPerspective_SKY = glGetUniformLocation(programID2, "projection");
...
void genereVBO ()
{
 ...
 glBindVertexArray(0);
  ...
  if(glIsBuffer(VBO_SKY) == GL_TRUE) glDeleteBuffers(1, &VBO_SKY);
    glGenBuffers(1, &VBO_SKY);
    glBindBuffer(GL_ARRAY_BUFFER, VBO_SKY);
    glBufferData(GL_ARRAY_BUFFER, sizeof(skyboxVertices),skyboxVertices , GL_STATIC_DRAW);

    glGenVertexArrays(1, &VAO_SKY);
        glBindVertexArray(VAO_SKY); // ici on bind le VAO , c'est lui qui recupèrera les configurations des VBO glVertexAttribPointer , glEnableVertexAttribArray...
            glEnableVertexAttribArray(indexVertex_SKY);
          

            glBindBuffer(GL_ARRAY_BUFFER, VBO_SKY);
            glVertexAttribPointer ( indexVertex_SKY, 3, GL_FLOAT, GL_FALSE, 0, (void*)0 );
...
//on envoie les données necessaires aux shaders */
 ...
 glUniformMatrix4fv(MatrixIDView_SKY, 1, GL_FALSE,&View[0][0]);

 glUniformMatrix4fv(MatrixIDPerspective_SKY, 1, GL_FALSE, &Projection[0][0]);

 glUniform3f(locCameraPosition,cameraPosition.x, cameraPosition.y, cameraPosition.z);


 glUniform1i(locationTextureSKY,0);

//pour l'affichagetexture
glActiveTexture(GL_TEXTURE0);
glBindTexture(GL_TEXTURE_2D,bufTexture0);

	glBindVertexArray(VAO_SKY); // on active le VAO
